...
?>
				<div class="col-xs-12" id="induk-barang">
						...
						<div class="row">
                            <button id="bootbox-regular"class="btn btn-white btn-primary ace-icon fa fa-plus-circle purple"> Tambah Data barang</button>
						...
						<div class="modal-dialog">
							<div class="modal-content">
								...
								<div class="modal-footer" style="background-color:#8F0000; color:white;">
									...
									<button data-bb-handler="confirm" type="button" class="btn btn-inverse" id="btn-ok">OK</button>
								</div>
							</div>
						</div>
				    </div>
	...
	$("#pg-2").click(function(event){
	event.preventDefault();
	});
	
	$("#pg-3").click(function(event){
	event.preventDefault();
	...
	$('#demo-group').append('<li><a href="#"><i class="ace-icon fa fa-angle-double-right"></i></a></li>');
       ...
       recreate_pagging2(arraypage[0+1]);
		...
		function ganti_page(val){
		//function ini berasosiasi dengan function pindah page
			val = val - 1;
			pindahpage(val,$("#page-start").val(),$("#page-finish").val());
		}			 
					  ...
					  function gettotalrecord(){
						...
						function cari_barang(myparam){
							...
							//alert('search sukses');
								...
								for (var i =0; i< 4; i++){
										...
										buat_pagingcari(obj[i]['totalrecord']);
										buat_info_tabel(obj[0]['rownum'],obj[i]['rownum'],obj[i]['totalrecord']);
						...
						function buat_pagingcari(total_record){
						$("#paginationbarang").empty();
								...
								var controllid=0;
									...
									index ++;
										$("#paginationbarang").append('<li class="next" ><a  id="'+'page'+controllid+'"  >'+index+'</a></li>');
										 ...
										 //alert($("#text-search-barang").val());
											pindahpagecari(this.id,$("#page-start").val(),$("#page-finish").val(),$("#text-search-barang").val());
											//alert(this.id);
									...
									//$("#paginationbarang").append('<li class="prev" id="'+'page'+index+'"><a>'+lastindex+'</a></li>');
									$("#paginationbarang").append('<li class="prev" id="'+'page'+index+'"><a>'+lastindex+'</a></li>');
						 ...
						 function pindahpagecari(page,pagestart,pagefinish,myparam){
							var pagenum=page.substring(4);
							//alert(pagenum);
							//alert(myparam);
							
							var targeturl="<?php echo base_url().'index.php/barang_controller/pindahpagecari'?>";
							$.ajax({
							url:targeturl,
							type: "POST",
							data:{
...
var my_global_var='';
	   ...
	   buat_paging($("#total_record2").val());
		...
		alert($('#total_record2').val());
		if(parseInt($('#total_record2').val()) >= 30){
		createsegment();
	   }
	...
	<!--<textarea id="test-data" cols=90 rows=10></textarea>-->